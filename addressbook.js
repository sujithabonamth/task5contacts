...
function showForm() {
  ...
  addButton.style.display = "block";//sets its display style property to "block," making the form visible.
...
function showUserDetails(index) {
  ...
  if (storedData && storedData[index]) {//Checks if there is stored data and if it contains information for the specified index.
      ...
      deleteIcon.onclick = function () {
        deleteContact(index);//removes a contact at the specified index from the stored data array in local storage.

        var detailsItem =displayDetails();//Invokes displayDetails function 
        var detailsList = document.getElementById("detailsList");
        if (detailsItem && detailsItem.parentNode === detailsList) {
          detailsList.removeChild(detailsItem);//removes the displayed details item from the details list.
        }
